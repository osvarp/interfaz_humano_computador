...
function DisplayProduct( { product } ) {
    ...
    const getImages = async () => {
        const prodImageRef = ref( getStorage(), "productImage/" + product.productImage )
        await getDownloadURL( prodImageRef ).then( (url) => {
            setProductImage( url );
        } );

        const q = query( collection( db, "Users" ), where( "username", "==", product.username ) );
        ...
        const sellProfRef = ref( getStorage(), "userImage/" + tmp.profileImage );
        const profImg = await getDownloadURL( sellProfRef ) ;
        setSellerUser( { ...sellerUser, seller:tmp.seller,phoneNumber:tmp.phoneNumber, profileImage:profImg } );
